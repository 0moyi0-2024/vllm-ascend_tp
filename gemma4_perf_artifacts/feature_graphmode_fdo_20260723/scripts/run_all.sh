...
# 2 slots: each owns 4 NPUs (TP4) + a port. FULL_DECODE_ONLY needs TP4 to split
# head_dim to 128 (avoids the 512-dim PA capture crash at TP1/TP2).
SLOT_DEVS=("0,1,2,3" "4,5,6,7")
SLOT_PORTS=("9000" "9002")

# Round-robin assign cases to 2 slots
declare -a SLOT_CASES_0 SLOT_CASES_1
for i in "${!CASES[@]}"; do
  slot=$((i % 2))
...
echo "[$(date '+%F %T')] run_all START, ${#CASES[@]} cases, 2 parallel slots (TP4 each)" | tee -a "${OUT}/logs/run_all.log"

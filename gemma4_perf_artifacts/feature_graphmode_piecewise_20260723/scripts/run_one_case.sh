...
# COMMON_FLAGS: graph-mode variant — use PIECEWISE cudagraph (avoids the
# FULL_DECODE_ONLY PA 512-dim capture crash ACL 107033) instead of --enforce-eager.
...
COMMON_FLAGS="--compilation-config '{\"cudagraph_mode\":\"PIECEWISE\"}' --max-model-len 8192 --block-size 128"
CMD="cd ${CWD} && ASCEND_RT_VISIBLE_DEVICES=${DEVICES} HCCL_OP_EXPANSION_MODE=AIV HCCL_BUFFSIZE=256 vllm serve ${MODEL_PATH} --served-model-name ${SERVED_NAME} --tensor-parallel-size 2 ${BASE_FLAGS} ${COMMON_FLAGS} ${FEATURE_FLAGS} ${ADD_ARG} --host 0.0.0.0 --port ${PORT}"
